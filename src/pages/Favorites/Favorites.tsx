...
const FavoritesPage = () => {
  ...
  const favoritePsychologists = useMemo(() => {
    return psychologists.filter((p) => favorites.includes(p.id));
  }, [psychologists, favorites]);

  const filteredAndSortedPsychologists = useMemo(() => {
    let result = [...favoritePsychologists];
  ...
  }, [favoritePsychologists, filterSort]);
    ...
    <div className="container">
      ...
      {loading ? (
        ...
        <p className={css.empty}>
          <span>You haven't added any psychologists yet </span>
          <svg width="20" height="20">
            <use href="/sprite.svg#icon-heart-green"></use>
          </svg>
        </p>
      ) : (
